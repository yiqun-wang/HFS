...
import numpy as np
from models.embedder_high import get_embedder
...
class SDFNetwork(nn.Module):
    ...
    def forward(self, inputs):
        ...
        if self.embed_fn_fine is not None:
            ...
            nfea_eachband = int(input_enc.shape[1] / self.multires)   # 6
            ...
            inputs = torch.cat([inputs, inputs_enc], dim=-1)  # [B,...,6L+3]
...
class SDFNetworkHigh(nn.Module):
    ...
    def forward(self, inputs, sdf_network, gradient, s):
        ...
        if self.embed_fn_fine is not None:
            ...
            input_enc = input_enc.view(-1, self.multires, nfea_eachband)[:, N:, :].view(
                [-1, self.num_eoc]).contiguous()
            input_enc = (input_enc.view(-1, N) * weight[N:]).view([-1, self.num_eoc])
            flag = weight[N:].tile(input_enc.shape[0], nfea_eachband, 1).transpose(1, 2).contiguous().view(
                [-1, self.num_eoc])
            inputs_enc = torch.where(flag > 0.01, inputs_enc, input_enc)
...
class NeRF(nn.Module):
    ...
    def forward(self, input_pts, input_views):
        ...
        if self.embed_fn_view is not None:
            ...
            input_views = torch.cat([input_views, input_views_enc], dim=-1)  # [B,...,6L+3]
